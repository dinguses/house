...
public class HouseManager : MonoBehaviour
{
	...
	public GradualTextRevealer gameText;
	...
	void SetupHouse()
	{
		...
		killerResponseGenerics.Add ("slap");
		killerResponseGenerics.Add ("smack");
		killerResponseGenerics.Add ("beat");
		killerResponseGenerics.Add ("hit");
		killerResponseGenerics.Add ("tussl");
		killerResponseGenerics.Add ("punch");
	...
	void Start()
	{
		...
		trackLength = ((GetClip (audioTracks.First ())).length) * 50;
		PlayKnockingClip (GetClip (23));

		pauseTrack.clip = Resources.Load ("track7") as AudioClip;
	...
	Dictionary<int, List<string>> GetLockdownOptions()
	{
		...
		dict.Add (0, new List<string> { "use [0]", "get [0]", "grab [0]", "pick up [0]", "stab [killer]", "use [0] on [killer]", "stab [killer] with [0]", "attack [killer] with [0]",
			"using [0]", "getting [0]", "grabbing [0]", "picking up [0]", "stabbing [killer]", "using [0] on [killer]", "stabbing [killer] with [0]", "attacking [killer] with [0]",
			"use [2]", "get [2]", "grab [2]", "pick up [2]", "use [2] on [killer]", "stab [killer] with [2]", "attack [killer] with [2]",
			"using [2]", "getting [2]", "grabbing [2]", "picking up [2]", "using [2] on [killer]", "stabbing [killer] with [2]", "attacking [killer] with [2]",
		...
		dict.Add (2, new List<string> { "use [0]", "threaten [0]", "threaten [0] with [1]", "threaten [0] with [2]", "threaten [0] with [3]", "threaten [0] with [4]", "threaten [0] with [5]",
			...
			"use [6]", "hack [6]",
			"using [6]", "hacking [6]",
			"destroy [0]", "decapitate [0]", "rip head off [0]", "rip head off of [0]", "tear head off [0]", "tear head off of [0]", "remove head from [0]",
			"run", "run away", "flee", "escape", "running", "running away", "fleeing", "escaping",
			"talk", "speak", "plead", "talk to [killer]", "talk with [killer]", "speak to [killer]", "speak with [killer]", "plead with [killer]", "reason with [killer]",
			"talking", "speaking", "pleading", "talking to [killer]", "talking with [killer]", "speaking to [killer]", "speaking with [killer]", "pleading with [killer]", "reasoning with [killer]",
			"make friends", "make friends with [killer]", "befriend", "befriend [killer]",
		...
		dict.Add (11, new List<string> { "[0]", "[1]", "basement", "back", "close [0]", "close [1]", "shut [0]", "shut [1]" });
		...
		dict.Add (15, new List<string> { "[0]", "[1]", "bedroom", "bathroom", "close [0]", "close [1]", "shut [0]", "shut [1]" });
		...
		dict.Add (20, new List<string> { "[0]", "[1]", "shed", "back", "close [0]", "close [1]", "shut [0]", "shut [1]" });
	...
	Texture2D GetRandomDeathImage()
	{
		...
		if (currRoom == 0 && policeTimer >= policeCap) {
			...
			} else if (imageName == "lrdeath2" || imageName == "lrdeath3") {
				GameOverAudio (156, true);
			}
			else {
	...
	void KillerResponseGeneric (string generic){
		if (generic.Contains ("run") || generic.Contains ("flea") || generic.Contains ("escape")) {
			...
			if (currentRoom.Index == 6) {
				...
				if (weapon == "gun") {
					...
					AddText("There isn't anywhere for you to run, as the killer stands in the way of the only exit. You run at him with the intent of pushing past him, but he heaves his flail squarely into your chest, shattering your ribs like they were porcelain. As you perish, you amuse yourself by ridiculing the killer for clearly being a major LARPing nerd. This happens only in your head however, as all you are able to muster are a few bloody gurgles.\n\nPress [ENTER] to try again.");
				}
			...
			if (currentRoom.Index == 7) {
				...
				if (weapon == "knife") {
					...
					SetImage (GetImageByName (overlays.ElementAt (UnityEngine.Random.Range (2, 4))));
					AddText ("You flee, as quietly as you can, towards the back door since the fence blocks any other exit. Despite these efforts, the man in the cleansuit detects you, and with nothing impeding his way, he runs towards you. He is unexpectedly fast, and you are quickly overrun. Though you struggle, the killer is still able to bash you to bits with an honest-to-goodness flail.\n\nPress [ENTER] to try again.");
				}
		...
		if (generic.Contains ("attack") || generic.Contains ("fight") || generic.Contains("punch") || generic.Contains("hit") || generic.Contains("tussl") || generic.Contains("slap") || generic.Contains("beat") || generic.Contains ("smack")) {
			...
			if (killerInBedroom) {
				...
				foreach (var option in options) {
					if (text == option) {
						...
						var revolverNames = GetAltNames ("revolver");

						foreach (var killerResponseGen in killerResponseGenerics) {
							if (text.Contains (killerResponseGen)) {

								bool pistolWhip = false;

								//TODO: Fix pistol whips
								foreach (var revolverName in revolverNames) {
									if (text.Contains (revolverName)) {
										pistolWhip = true;
										text = "pistol whip";
									}
								}
				...
				if (!playerLairThreaten) {
					...
					string killerText = "Suddenly, you hear a loud grinding and sliding, which startles you into a state of panic. The false panel of the fireplace has moved. You back away towards the other wall. To your horror, you see the the feet of the clean suit begin to descend the stairs into the room, followed by the rest of the murderer.\n\nWhat do you do?";
							...
							if (currLockdownOption == 2) {
								...
								if (text.Contains ("scalpel")) {
									...
									if (text.Contains ("use") || text.Contains("using")) {
										...
										if (text.Contains ("destroy") || text.Contains("decapitate") || text.Contains("rip") || text.Contains("tear") || text.Contains("remove")) {
											LairThreaten ("destroy");
										}
	...
	bool ConfrontationPause (string text) {
		...
		if (confrontPause) {
			if (text.Contains ("back") || text.Contains("return") || text.Contains("leave")) {
	...
	void GameOverAudio(int lossToPlay, bool loss) {
		...
		if (loss) {
			...
			// Play Loss 5
			else if (lossToPlay == 151 || lossToPlay == 152 || lossToPlay == 153 || lossToPlay == 156) {
				lossTrack.clip = GetClip (111);
			}
			...
			// Play Loss 3
			else if (lossToPlay == 101 || lossToPlay == 124 || lossToPlay == 154) {
			...
			if (trapStep3) {
				...
				if (bearTrap) {
					...
					if (playerKnowsBeartrap) {
						if (bedSpring) {
							AddText ("Using what you learned from the home bear survival guide, you get to work. You take the spring from your mattress and firmly attach the knife to it, creating a trap...of sorts. You aren't sure if it would work against a bear, but it might mangle that man's leg.");
						}
						else {
						...
						if (bedSpring) {
							AddText ("Using what you learned from a bear attack home survival guide, you get to work. You take the spring from your mattress and firmly attach the knife to it, creating a trap...of sorts. You aren't sure if it would work against a bear, but it might mangle that man's leg.");
						} 
						else {
	...
	void AdvanceMultiSequence(){
		...
		if (step.Key == "randomdeath") {
			...
			if (step.Key == "trapstairs" && bedSpring) {
				stepValue = "The jaws of the makeshift bear trap clamp down on his leg with surprising force, driving the blades deep into his leg. That was one damn good mattress; it's bravery will not be forgotten.";
			}
	...
	public void RandomizeTracks(){
		...
		trackNumbers.Add (63);
		trackNumbers.Add (64);
		trackNumbers.Add (155);

		int trackNumLength = trackNumbers.Count;

		//trackNumbers.Add (122);

		for (int x = 0; x < trackNumLength; ++x) {
	...
	public void AddText(string txt)
	{
		...
		if (!inventoryUp && !helpScreenUp && !multiSequence) {
			if (image.sprite.name == "blankoverlay") {

				//TODO: Resore image AND OVERLAY upon returning from help/inv screens w/ bad input


				if (storedImage != "") {

					if (storedOverlay != "" && storedOverlay != "blankoverlay") {
	...
	public void AddAdditionalText(string txt)
	{
		...
		gameText.AddAdditionalText (txt);
	}
	...
	void SetImage(Texture2D tex)
	{
		...
		if (tex.name == "orangejuice" || tex.name == "icecubetrays" || tex.name == "flashlight" || tex.name == "bleach" || tex.name == "bucket"
			|| tex.name == "phone3" || tex.name == "phone4" || tex.name == "phone5" || tex.name == "bedrtable5" || tex.name == "bedrtable6" || tex.name == "gun" || tex.name == "poster") {
			twoLayerLook = true;
		} else {
			twoLayerLook = false;
		}
		...
		if (tex.name.Contains ("invbig")) {
			if (image.sprite.name != currentRoom.currentState.Image && !image.sprite.name.Contains("invbig")) {
				if (image.sprite.name != "blankoverlay") {
		...
		if ((image.sprite.name == "phone4" || image.sprite.name == "phone5") && (tex.name != "phone4" && tex.name != "phone5" && tex.name != "blankoverlay" && !tex.name.Contains("invbig") && !tex.name.Contains("hw")
			&& !tex.name.StartsWith("k") && !tex.name.StartsWith("lab") && !tex.name.Contains("death") && !tex.name.Contains("tombstone") && !tex.name.Contains("firedeath"))) {
	...
	void LairThreaten (string weapon) {
		switch (weapon) {
		case "destroy":
			SetImage (GetImageByName ("beardestroy"));
			AddText("You destroy the teddy bear and he gets real upset. MAYBE YOU SHOULDN'T DO THAT!\n\nPress [ENTER] to restart.");
			GameOverAudio (-1, true);
			health = 0;
			break;
		case "scalpel":
	...
	void LockdownResponse(string text, bool valid) {
		...
		case 8:
			if (valid) {
				if (text.Contains ("1") || text.Contains ("one") || text.Contains("al") || text.Contains("gore")) {
					OtherCommands ("read book 1");
					return;
				} else if (text.Contains ("2") || text.Contains ("two") || text.Contains("secret")) {
					OtherCommands ("read book 2");
					return;
				} else if (text.Contains ("3") || text.Contains ("three") || text.Contains("bear")) {
					OtherCommands ("read book 3");
					return;
				} else if (text.Contains ("4") || text.Contains ("four") || text.Contains("murdered")) {
	...
	void CombineItems() {
		...
		case 4:
			if (dummyStepsCompleted == 1) {
				AddText ("You put the piñata on top of your effigy. Now you just need to lure the killer into here.");
			}
			else {
				AddText ("You put the piñata on top of your effigy. Finally, it is complete! Better get out of sight before the killer gets here.");
			}
			RemoveFromInv (67);
			ChangeState (102, 1);
	...
	public void Look(List<string> argv = null)
	{
				...
				if (currentRoom.Index == 5 && (bearTrapMade || bucketTrapMade || fireTrapMade || shitOnStairs || blenderTrapMade)) {
					...
					if (hideKillerInKitchen) {
						...
						if (overlayImage.sprite.name.Contains ("katana")) {
							if (diedByHiding)
								AddText ("As you emerge from your hiding spot, you find the killer standing in the middle of the room; he has come for you at last.\n\nPress [ENTER] to continue.");
							else
								AddText ("You find the killer in the next room; he has come for you at last.\n\nPress [ENTER] to continue.");
						} else if (overlayImage.sprite.name.Contains ("knife")) {
							if (diedByHiding) 
								AddText ("Just as you emerge from your hiding place, you find the killer standing poised with a knife - ready to strike.\n\nPress [ENTER] to continue.");
							else
							...
							if (diedByHiding)
								AddText ("When you step out from your hiding place, your heart skips a beat as you see the man in the cleansuit already beginning to take aim at your face with a silenced pistol.\n\nPress [ENTER] to continue.");
							else
							...
							if (diedByHiding) 
								AddText ("Stepping out from your hiding spot, you recoil in shock as you see the imposing figure of the man in the cleansuit wielding a flail.\n\nPress [ENTER] to continue.");
							else
								AddText ("You recoil in shock as you see that, in the next room, the imposing figure of the man in the cleansuit stands, wielding a flail.\n\nPress [ENTER] to continue.");
						}
					}
		...
		if (argv [1] == "behind") {
			...
			if (underObj != null) {
				if (underObj.Index == 54 || underObj.Index == 4) {
		...
		if (itemName == "book" && currentRoom.Index == 0) {

			if (image.sprite.name == "phone2" || image.sprite.name == "phone3" || image.sprite.name == "phone5") {
				Look ("look address book".Shlex ());
				return;
			} 
			else if (image.sprite.name != "undercushion") {
	...
	public void ResetItemGroup(){
		...
		if (currentItemGroup != null) {
			foreach (int item in currentItemGroup.Items) {
				...
				if (currentItemGroup.BaseItemIndex == 51) {
					var obj = GetObjectByName ("closet");

					if (obj.State != 0) {
						AudioClip clipToPlay = GetClip (89);
						toPlaySoundFX.Add (clipToPlay);
					}
				}

				if (currentItemGroup.BaseItemIndex == 62 || currentItemGroup.BaseItemIndex == 63) {
					var wObj = GetObjectByName ("washer");
					var dObj = GetObjectByName ("dryer");

					if (wObj.State != 0) {
						AudioClip clipToPlay = GetClip (98);
					...
					if (dObj.State != 0) {
						AudioClip dClipToPlay = GetClip (100);

						if (!toPlaySoundFX.Contains (dClipToPlay))
							toPlaySoundFX.Add (dClipToPlay);
					}
	...
	void PlayClips()
	{
		...
		if (toPlaySoundFX.Count != 0 && !audioQueueLocked) {
	...
	public void UpdateTimers()
	{
		if (skipUpdate) {
			...
			if (!killerInKitchen) {
				...
				if (policeTimer == policeCap && !policeArrived) {
					policeArrived = true;
	...
	public void Move(List<string> argv)
	{
		...
		if (firstWordGo) {
			if (!roomName.Contains("under") && firstWordGo && (roomName.Contains("sleep") || (roomName.Contains("bed") && !roomName.Contains("bedr") && !roomName.Contains("side") ))){
				OtherCommands ("sleep");
				return;
			}

			if (roomName.Contains ("out")) {
				if (currentRoom.Index == 0 && roomName.Contains("front")) {
		...
		if (newRoomObj != null) {
			...
			if (isRoom) {
				...
				if (altNameId == currentRoom.Index && altName.Contains (roomName)) {
					if (currentRoom.Index == 1 && altName.Contains ("up")) {
			...
			if (newRoomObj.currentState.Gettable == 1) {
				...
				if (rooms [newRoom].Index == 6) {
					if (!killerInKitchen) {
						...
						AddText ("You can't go in there, there's a guy to murder!");
						return;
					}
				}
				...
				if (newRoom == 0 && currentRoom.Index == 1) {
					StopLoopingAudio ();
				}
	...
	public void Get(List<string> argv)
	{
		...
		// Get up from "sitting" in arm chair
		if (image.sprite.name.Contains ("armchair") && (itemName.Contains("up") || itemName.Contains("out"))) {
			OtherCommands ("back");
			return;
		}
		...
		if (item.currentState.Gettable == 1) {
			...
			if (item.Index == 54) {
				...
				/*if (item.State == 0) {
					var washerObj = GetObjectByName ("washer");
					ImageCheckAndShow (washerObj.Index, washerObj.State, washerObj.currentState.Image);
					AddText ("GOTAA OPEN IST FIRST IDIOT");
					return;
				} else {*/
				Use ("use dryer".Shlex ());
				return;
	...
	public void Use(List<string> argv)
	{
		...
		foreach (var response in useResponses) {
			...
			if (response.ItemIndex == 151) {
				Use ("use computer".Shlex ());
				return;
			}
	...
	public void OtherCommands(string text)
	{
		...
		case "wear":
			...
			if (currentRoom.Index == 8) {
				...
				if (itemName.Contains ("rake") && itemName.Contains ("tarp")) {
					Use ("use tarp with rake".Shlex ());
					return;
				}
				...
				if (isDummy && isPinata) {
					Use ("use pinata with dummy".Shlex ());
					return;
				}
		...
		case "take":
			...
			if (itemName.Contains ("shit") || itemName.Contains ("crap") || itemName.Contains ("dump") || itemName.Contains ("poop")) {
				OtherCommands ("shit");
				return;
			}
		...
		case "pick":
			if (itemName.Contains (" up") || itemName.Contains ("up ")) {
				...
				if (currentRoom.Index == 0 && (itemName.Contains ("phone") || itemName.Contains ("receiver"))) {
					Use ("use phone".Shlex ());
					return;
				}
		...
		/*case "knock":
			if (currentRoom.Index == 0) {
				...
				ResetItemGroup ();
				UpdateRoomState (false);
				SetImage (GetImageByName (frontDoor.currentState.Image));

				AddText ("After pausing for a moment, you decide to knock on the door back at the man in the cleansuit. There is no reply.");
				return;
			}
		...
		case "giggle":
		case "laugh":
			AddText ("You manage to choke out a panicked giggle. More like a squeal actually, but it'll have to do.");
			return;
		case "beat":
		case "jerk":
		case "jack":
			if (itemName.Contains ("off") || itemName.Contains ("it") || itemName.Contains("meat") || itemName.Contains("dick") || itemName.Contains("penis") || itemName.Contains("cock")) {
				OtherCommands ("masturbate");
				return;
			}
		...
		case "wank":
		case "masturbate":
			switch (jerkOffCounter) {
			case 0:
				AddText ("...Oh, uh...hmm. Well, you see, uh...no. That's a no. Please don't ask again.");
				break;
			case 1:
				AddText ("No, I’m serious, stop asking that. Just no.");
				break;
			case 2:
				AddText ("How many times do I have to say this? No.");
				break;
			case 3:
				AddText ("...Ugh, fine. You pull down your pants and - NO. You don’t do anything. Nothing happens. Stop trying to jerk off.");
				break;
			case 4:
				AddText ("Last warning: if you try to masturbate any more, you will fucking die.");
				break;
			case 5:
				KillSelf ("Well, here we are. You're dead. You died trying to jack it.");
				return;
			}
			jerkOffCounter++;
			return;
		case "stand":
			if (image.sprite.name.Contains ("armchair") && (itemName.Contains ("up"))) {
				OtherCommands ("back");
				return;
			}
		...
		case "add":
			...
			break;
		case "pause":
			if (itemName == "" || itemName.Contains ("game")) {
				var pauseFadeColor = pauseFade.color;
				pauseFadeColor.a = 0.0f;
				pauseFade.color = pauseFadeColor;
		...
		case "set":
			if (currentRoom.Index == 0) {
				...
				if (itemName.Contains ("fire")) {
					if (currentRoom.Index != 6) {
						if ((IsInInv (15) || IsInInv (64)) && IsInInv (43)) {
							room = 0;
							Use ("use fireplace".Shlex ());
							return;
						} else {
			...
			return;
		case "build":
			if (itemName.Contains ("trap") && currentRoom.Index == 5) {
				Use ("use workbench".Shlex ());
				return;
			}
	...
	void Cry() {
		AddText ("The stress of the situation is just too much and you struggle to hold back a few sobs. Hang in there buddy, you can do it! Don’t forget about the HELP screen!");
		//PlayClip (GetClip (76));
	}

	void Sing() {
		AddText ("Apparently unafraid of giving up your location to the man at the door, you belt out a few lines of your favorite song.");
		//PlayClip (GetClip (76));
	}
	...
	public void Turn(string text){
		...
		if ((turnOnResponses.Count() == 0 && turnOn) || (turnOffResponses.Count() == 0 && turnOff)) {
			if (turnOn) {
	...
	public void Read(int i, int j)
	{
		...
		if (j != -1)
		{
			...
			if ((item.Index == 17 || item.Index == 18 || item.Index == 19 || item.Index == 20) && killerInKitchen) {
				AddText ("There isn't any time to read anything now! The killer is critically wounded in the kitchen! Now is your chance! GO!");
				return;
			}
